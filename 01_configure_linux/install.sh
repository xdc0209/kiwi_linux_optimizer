...
# 执行一级子目录下的安装脚本。##
for install_script in $(find $BASE_DIR -mindepth 2 -maxdepth 2 -name "install.sh" | sort)
do
    print_info "Exe [ $install_script ] start."
    bash $DEBUG_SWITCH $install_script 2>&1 | tee -a $log
    [ "${PIPESTATUS[0]}" -ne 0 ] && die "Exe [ $install_script ] fail."
    print_info "Exe [ $install_script ] finish."
done
