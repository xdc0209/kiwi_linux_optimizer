...
# 添加sudo日志配置##
################################################################################################################################
[ $UID -eq 0 ] && [ -z "$(grep '^Defaults logfile=' /etc/sudoers)" ] && {
    sed -i '/^Defaults.*env_reset/a Defaults logfile=/var/log/sudo.log' /etc/sudoers
    touch $BASE_DIR/sudoers_installed_flag
}
